...
| `--insecure-options` |  none | - **none**: All security features are enabled </br> - **http**: Allow HTTP connections. Be warned that this will send any credentials as clear text. * **image**: Disables verifying image signatures * **tls**: Accept any certificate from the server and any host name in that certificate * **ondisk**: Disables verifying the integrity of the on-disk, rendered image before running. This significantly speeds up start time. * **all**: Disables all security checks | Comma-separated list of security features to disable |
